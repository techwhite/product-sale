...
public class ProductServiceTest {
    ...
    public void testSaveProduct() {
        ProductEntity expected = new ProductEntity("testSaveProduct", 100f);

        // todo: delete first for other test cases
        // 0. clear test data
        productService.deleteProductByName(expected.getName());

        // 1. save new one
        ProductEntity result = productService.createProduct(expected);
        ...
        Exception exception = assertThrowsExactly(DuplicateKeyException.class, () -> productService.createProduct(expected));
        assertEquals("Already exist product for name: " + expected.getName(), exception.getMessage());
    }
